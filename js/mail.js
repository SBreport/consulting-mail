...
const MAIL_TEMPLATES_V2 = {
    ...
    // ----------------------------------------------------------
    direction: {
        '대행사 창업': (name) => `수익화 방향으로 **대행사 창업**을 택하셨죠. 수익의 천장이 높고 사업 자산이 내 것으로 쌓인다는 점에서 잘 고르셨습니다.
...
한 가지 꼭 짚어 드리고 싶은 점이 있습니다. 병원 원장님을 단순 고객으로 모집하려 하면 어렵습니다. 블로그 마케팅을 포함해서 병원이 겪고 있는 마케팅 문제를 함께 풀어준다는 마인드로 접근하는 것이 핵심입니다.`,
        ...
        '원고작가': (name) => `수익화 방향으로 **원고작가 활동**을 선택하셨네요. 초기 투자 없이 바로 시작할 수 있고, 실력을 쌓으면서 수익도 함께 만들 수 있는 구조입니다.
...
5) 클라이언트와 담당 업무를 조금씩 넓혀가시길 권합니다`,
        ...
        '병행': (name) => `**대행사 창업과 원고작가 활동 병행**을 목표로 잡으셨군요. 처음부터 대행사로 뛰어드는 것은 부담이 클 수 있기 때문에 가장 현실적인 접근이라 생각합니다.
...
4) 기존 클라이언트 외에 추가 대행사로 업무를 전환해 가시길 권합니다`,
        ...
        '병행_운영중': (name) => `**대행사 창업과 원고작가 활동 병행**을 목표로 하셨고, 이미 대행사를 운영 중이시죠. 글 작성부터 클라이언트 관리까지 직접 경험하고 계신 것 자체가 큰 자산입니다.
...
4) 신규 계약을 따내면서 상품 업셀링도 점검해 보시길 권합니다 (최적블 or 플레이스)`,
        ...
        '병행_활동중': (name) => `**대행사 창업과 원고작가 활동 병행**을 목표로 하셨는데, 현재 원고작가로 활동 중이시니 업종별 글쓰기, 키워드 기획, 배포까지 이미 감이 잡혀 있으실 겁니다. 그 경험이 곧 대행사 창업의 든든한 베이스가 됩니다.
...
5) 신규 계약을 따내면서 업셀링도 점검해 보시길 권합니다 (최적블 or 플레이스)`,
        ...
        '미정': (name) => `수익화 방향은 아직 탐색 중이시군요. 선택을 돕기 위해 두 가지를 먼저 비교해 드리겠습니다.
...
대부분의 경우, 원고작가로 먼저 시작해 시장 감을 익힌 뒤 창업 여부를 결정하는 것이 가장 안전한 경로입니다.`
    ...
    // ----------------------------------------------------------
    startPoint: {
        '준비 중': (name) => `현재 상황을 **창업/활동을 준비 중**이라고 하셨으니, 아직 기초 단계이시네요. 차근차근 진행하시면 충분합니다.
...
완벽하게 준비한 뒤 시작하려 하면 계속 미뤄지기 마련입니다. 일단 실행하면서 보완해 나가는 방식으로 접근하셔야 성과가 납니다.`,
        ...
        '운영 중': (name) => `**이미 대행사를 운영 중**이시니 기초 단계는 건너뛰셔도 됩니다. 지금은 운영 효율을 높이는 데 집중하실 때입니다.
...
• 서비스 패키지도 한 단계 업그레이드해 보시길 권합니다`,
        ...
        '활동 중': (name) => `**원고작가/프리랜서로 활동 중**이시니 실무 감각은 충분히 갖추고 계십니다. 이제 그 경험을 무기로 다음 단계를 열어가야 할 때입니다.
...
• 원고작가 이력을 살려 1:1 케어 맞춤형 서비스를 무기로 영업을 시작해 보시길 권합니다`,
        ...
        '부업': (name) => `**직장을 다니시면서 부업으로 준비 중**이시죠. 시간이 제한되어 있는 만큼 효율이 무엇보다 중요합니다.
...
• 인근 병원에 제안서를 배포하고 미팅 일정을 잡아 보시길 권합니다`
    ...
    // ----------------------------------------------------------
    solution: {
        '첫 고객': (name) => `가장 큰 고민으로 **첫 고객(병원) 확보**를 꼽으셨죠. 가장 현실적인 방법은 '마케팅(블로그) 작업이 중단된 곳'을 타겟으로 잡는 것입니다.
...
5) 무료 1개월 마케팅을 먼저 실행해 드린 뒤 계약을 제안하시길 권합니다`,
        ...
        '서비스/가격': (name) => `**서비스 구성 및 가격 책정**이 가장 어렵다고 하셨는데, 이건 '3단계 패키지'로 정리하시면 훨씬 명확해집니다.
...
3) 단순히 올리기보다 고객이 납득할 수 있는 이유가 필요합니다. 예를 들면 원고 작업 외에 디자인 작업을 추가로 해드리거나, 플레이스 관리나 악플 관리 업무를 얹어드리는 식으로요`,
        ...
        '영업/제안': (name) => `**영업 및 제안 방법**이 가장 큰 과제라고 하셨죠. 병원 영업은 '가치 선제공' 방식이 가장 효과적입니다.
...
핵심은 "팔려고 한다"가 아니라 "도와주려 한다"는 포지셔닝입니다. 이 한 가지가 결과를 바꿉니다.`,
        ...
        '실무 역량': (name) => `**실무 역량 강화**를 지금 가장 큰 과제로 꼽으셨네요. 그렇다면 원고작가로 먼저 시작하시길 권합니다.
...
6개월 정도 활동하고 나시면 직접 고객을 받을 역량이 갖춰져 있을 것입니다.`,
        ...
        '시간 관리': (name) => `**시간 관리 및 본업과의 병행**이 어렵다고 하셨죠. 이 경우 핵심은 '주 단위 루틴'을 먼저 만드는 것입니다.
...
처음 3개월은 성과보다 '습관 만들기'에 집중하시길 권합니다. 루틴이 자리 잡히면 그다음부터는 생각보다 빠르게 굴러갑니다.`,
        ...
        '방향': (name) => `**전체적인 방향 설정**이 아직 어렵다고 하셨군요. 다음 두 가지 질문에 먼저 답해 보시면 방향이 잡힐 겁니다.
...
3) 레퍼런스 쌓기 — 첫 1건이 정말 모든 것의 시작입니다`
    ...
    // ----------------------------------------------------------
    actionPlan: {
        '1개월': (name) => `목표 시점을 **1개월 내**로 잡으셨죠. 그렇다면 주 단위로 빠르게 움직이셔야 합니다.
...
• 4주차: 미팅 진행 + 첫 계약 목표로 실행하시길 권합니다`,
        ...
        '3개월': (name) => `목표 시점을 **3개월 내**로 잡으셨죠. 그렇다면 월 단위로 나눠 진행하시면 됩니다.

• 1개월 차: 강의 수강하면서 서비스·포트폴리오·리서치 기반 구축하기
• 2개월 차: 영업 시작 (타겟 30곳, 방문 후 제안서 배포 권장)
• 3개월 차: 첫 고객 확보를 목표로 하시길 권합니다`,

        '6개월': (name) => `목표 시점을 **6개월 내**로 잡으셨네요. 시간이 충분하니 제대로 기반을 다지며 나가실 수 있습니다.
...
• 4단계: 레퍼런스 구축하면서 서서히 확장하기`
    ...
    // ----------------------------------------------------------
    closing: {
        '혼자 실행': (name) => `자료를 바탕으로 **직접 실행**해 보시겠다고 하셨죠. 위 내용을 참고해 진행해 보시고, 막히는 부분이 생기면 편하게 연락 주시길 바랍니다.
• 최형기 대표 1:1 채널: https://open.kakao.com/o/sm0Cf1Kh
`,

        '피드백': (name) => `실행하면서 **피드백**을 받고 싶으시다고 하셨는데, 언제든 환영합니다.
...
`,
        ...
        '커뮤니티': (name) => `**커뮤니티**를 통해 함께 성장하고 싶다고 해주셨으니, 함께할 그룹을 안내해 드리겠습니다.
수강생 네트워크와 업종별 채널을 모아 뒀으니 활용하시길 바랍니다.
...
강의를 마치신 지도 어느덧 6개월이 지났네요. 얼마 전 아마겟돈에서 보내드린 **메타 광고** 강의는 잘 받아보셨는지요? 그동안 현장에서 직접 부딪혀 보며 느낀 점도, 막혀서 멈춰 계신 부분도 있으실 텐데요. 이번 설문에 남겨주신 내용을 토대로 ${name}님의 지금 상황을 점검하고, 다음 단계를 정리해 드리겠습니다.`;
        ...
        // 항목 요약 조립
        const targetItem = isTargetUndecided
            ? `• 주요 타겟 — 아직 정하지 않으셨어요`
            : `• 주요 타겟 — **${target}**`;
        const concernItem = isConcernUndecided
            ? `• 가장 큰 고민 — 아직 정리 중이에요`
            : `• 가장 큰 고민 — **${concern}**`;

        const hasUndecided = isTargetUndecided || isConcernUndecided;
        const undecidedHint = hasUndecided
            ? ' 타겟과 고민은 아래 내용을 함께 보시며 정해 가셔도 좋습니다.'
            : '';
...
function generateIntroHtml(name, status, monetization, target, concern, version = 'v2') {
    ...
    if (version === 'v1') {
        ...
        // --- v2: 2차 새 문구 ---
        const targetItemHtml = isTargetUndecided
            ? '• 주요 타겟 — 아직 정하지 않으셨어요'
            : '• 주요 타겟 — **' + target + '**';
        const concernItemHtml = isConcernUndecided
            ? '• 가장 큰 고민 — 아직 정리 중이에요'
            : '• 가장 큰 고민 — **' + concern + '**';

        const hasUndecidedHtml = isTargetUndecided || isConcernUndecided;
        const undecidedHintHtml = hasUndecidedHtml
            ? ' 타겟과 고민은 아래 내용을 함께 보시며 정해 가셔도 좋습니다.'
            : '';
